...
function PopupWithForm({name, title, buttontext, children, isOpen, onClose, onSubmit}) {
    ...
    return (
        <div className={`popup popup_type_${name} ${checkPopup()}`}>
            ...
            <form className={`popup__container popup__container_${name}`} onSubmit={onSubmit}>
                ...
                {children}
                <button type="submit" className="popup__submit popup__submit_active">{buttontext}</button> 
            </form>
        </div>
    )
}
